...
learn_new_models <- T # choose if new models have to be learned
source(file = "2.3_clin_vs_clin_miRNA_RF.R")
...
learn_new_models <- T # choose if new models have to be learned
# Note that for mRNA, models are learned only when the percentage
# of patients in the training set is 80% or the degradation factor is 1 
# (no degradation of the sequencing depth). 
source(file = "3.1_degrade_data_EN.R")

# RF !!! the following scrip can be time demanding !!!
learn_new_models <- T # choose if new models have to be learned
# Note that for random forest (both miRNA and mRNA), models are learned 
# only when the percentage of patients in the training set is 80% or the 
# degradation factor is 1 (no degradation of the sequencing depth). 
source(file = "3.2_degrade_data_RF.R")
...
# Note that in the paper, we degraded by factor 2 and 5 the sequencing depth 
# of cancers for which it is impossible to degrade by a factor 10.
# This step is not done here
signif_level <- 0.1 # 0.05 in the article

source(file = "3.4_sum_up_deg_tables.R")

# comparison of prediction obtanined with miRNA-seq and mRNA-seq data
source(file = "3.5_comp_pred_miRNA_mRNA.R")
...
learn_new_models <- T
source(file = "4.2_why_C_degraded_EN.R")
