...
my_plot <- function(df_final, df_final_mRNA, n_genes_df_final, count, count_mRNA, title = NULL,
  ...
  for(i in 1:(ncol(prop_df)-1)){
    res1 <- try(p_val_tmp <- wilcox.test(prop_df[,ncol(prop_df)], prop_df[,i], 
  ...
  for(i in 1:(ncol(prop_df_mRNA)-1)){
    res2 <- try(p_val_tmp <- wilcox.test(prop_df_mRNA[,ncol(prop_df_mRNA)], prop_df_mRNA[,i], 
                        alternative = alternative)$p.value)
  ...
  for(i in 1:(ncol(pat_df)-1)){
    res3 <- try(p_val_tmp <- wilcox.test(pat_df[,ncol(pat_df)], pat_df[,i], 
                             alternative = alternative)$p.value)
  ...
  for(i in 1:(ncol(pat_df_mRNA)-1)){
    res4 <- try(p_val_tmp <- wilcox.test(pat_df_mRNA[,ncol(pat_df_mRNA)], pat_df_mRNA[,i], 
                             alternative = alternative)$p.value)
  ...
  n_pat_vect <- rep(NA, 3)
  ind <- 1
  for(i in perc_train[c(1, length(perc_train) / 2, length(perc_train))]){
  ...
  pat_ggplot <- ggplot() +
    ...
    geom_text(aes(x = c(1, round(length(perc_train) / 2), length(perc_train)), 
                  y = rep(y_n_pat, length(n_pat_vect)), 
