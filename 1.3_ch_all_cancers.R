...
clinical_feat <- c("n patients", "p miRNA", "p mRNA", "Censoring rate", "Survival - 3 years", 
                   "C-index - EN", "C-index - RF", "Library size - miRNA", "Library size - mRNA")
...
for(cancer in cancers_all){
  ...
  data_ch[cancer, "Library size - miRNA"] <- signif(median(lib_size_miRNA[[cancer]]) / 10^6, 1)
  data_ch[cancer, "Library size - mRNA"] <- signif(median(lib_size_mRNA[[cancer]]) / 10^6, 1)
