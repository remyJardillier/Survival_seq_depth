...
if(learn_new_models){
  ...
  for(cancer in cancers_final){
    ...
    for(i in 1:n_rep){
      ...
      for(k in 1:K_folds){
        ...
        res <- try(fit <- learn_models(logCPM_sub_train_std, clin_train, "EN"))
        
        if(!inherits(res, "try-error")){
          
          # compute the C-index and the IBS 
          C_IBS_tmp <- C_IBS_func(fit, clin_train, clin_test, logCPM_sub_train_std, 
                                    logCPM_sub_test_std, "EN")
        ...
        res <- try(fit <- learn_models(logCPM_MEG_train_std, clin_train, "EN"))
        
        if(!inherits(res, "try-error")){
          
          # compute the C-index and the IBS - choose if the data have to
          # be used standardized or not
          C_IBS_tmp <- C_IBS_func(fit, clin_train, clin_test, logCPM_MEG_train_std, 
                                    logCPM_MEG_test_std, "EN")
  ...
  save(C_list_testH, IBS_list_testH, n_genes_list_testH,
       file = "data_fit/testH_deg_EN.RData")
  print("Data saved in: data_fit/testH_deg_EN.RData")
}


# Test if the differences are significative -------------------------------

load(file = "data_fit/testH_deg_EN.RData")
...
n_genes_sub_df <- data.frame(matrix(ncol = length(cancers_final), nrow = n_rep * K_folds))
colnames(n_genes_sub_df) <- cancers_final

n_genes_all_df <- data.frame(matrix(ncol = length(cancers_final), nrow = n_rep * K_folds))
...
why_C_deg_plot <-  ggplot() +
  ...
  geom_text(aes(x = 1:length(C_list_testH), y = rep(1, length(C_list_testH)), 
                label = stars_H1_C), col = "#7fc97f", size = 5) +
  ...
  geom_text(aes(x = 1:length(C_list_testH), y = rep(0.3, length(C_list_testH)), 
                label = n_med_genes), size = 5) +
  scale_fill_manual(values=c("#7fc97f","#fdb462","#386cb0")) +
  geom_bracket(xmin = length(cancers_final) + 0.05, xmax =length(cancers_final) + 0.25, 
               y.position = 0.92, label = "", col = "#386cb0", size = 1) +
  geom_bracket(xmin = length(cancers_final) - 0.25, xmax = length(cancers_final) - 0.05, 
               y.position = 0.92, label = "", col = "#7fc97f", size = 1)

print(why_C_deg_plot)

ggsave(ggarrange(why_C_deg_plot, labels = "A"), 
       filename = "pdf/why_C_deg_plot_miRNA_EN.pdf")

print("Figure saved in: pdf/why_C_deg_plot_miRNA_EN.pdf")
...
why_IBS_deg_plot <- ggplot() +
  ...
  theme_Publication() + theme(axis.text.x = element_text(size = 13)) +
  geom_text(aes(x = 1:length(IBS_list_testH), y = rep(0.5, length(IBS_list_testH)), 
                label = stars_H1_IBS), col = "#7fc97f", size = 5) +
  geom_text(aes(x = 1:length(C_list_testH), y = rep(0.47, length(C_list_testH)), 
                label = stars_H2_IBS), col = "#386cb0", size = 5) +
  geom_text(aes(x = 1:length(C_list_testH), y = rep(0, length(C_list_testH)), 
                label = n_med_genes), size = 5) +
  scale_fill_manual(values=c("#7fc97f","#fdb462","#386cb0")) +
  geom_bracket(xmin = length(cancers_final) + 0.05, xmax =length(cancers_final) + 0.25, 
               y.position = 0.42, label = "", col = "#386cb0", size = 1) +
  geom_bracket(xmin = length(cancers_final) - 0.25, xmax = length(cancers_final) - 0.05, 
               y.position = 0.42, label = "", col = "#7fc97f", size = 1)

print(why_IBS_deg_plot)

ggsave(ggarrange(why_IBS_deg_plot, labels = "B"), 
       filename = "pdf/why_IBS_deg_plot_miRNA_EN.pdf")
print("Figure saved in: pdf/why_IBS_deg_plot_miRNA_EN.pdf")
